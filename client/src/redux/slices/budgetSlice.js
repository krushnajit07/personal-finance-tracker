...
const budgetSlice = createSlice({
  ...
  reducers: {
    setBudget: (state, action) => {
      ...
      state.monthlyBudget = payload.monthlyBudget || 0
      state.categories = payload.categoryBudgets || []
      state.remaining = payload.remaining || 0
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchBudget.fulfilled, (state, action) => {
        ...
        state.remaining = action.payload?.remaining || 0
      })
      .addCase(fetchBudget.rejected, (state, action) => {
        state.loading = false
        state.error = action.payload || 'Failed to load budget'
      })
